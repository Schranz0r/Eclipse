...
public class XE_Object2D {
	...
	public void enableCollision(){
		this.getRect().setBounds(0, 0, (int)(width*scale), (int)(height*scale));
	...
	public void draw(int x, int y){
		...
		if(isTextured){
			...
			// use nearest filetring, pixel for days :D
	...
	public void createListObject(){
		...
		glBegin(GL_QUADS);
			...
			glVertex2f(width*scale, 0);		// top-right
			
			if(isTextured)
				glTexCoord2f(imageloader.texture.getWidth(), imageloader.texture.getHeight());
			glVertex2f(width*scale, height*scale);	// bottom-right
			
			if(isTextured)
				glTexCoord2f(0f, imageloader.texture.getHeight());
			glVertex2f(0, height*scale);		// bottom-left
